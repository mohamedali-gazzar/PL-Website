...
/**
 * Opening scene — the Powerline "P" is the power source, centred on screen.
 * It grows in, charges with energy inside its own pathways, then conductors
 * emerge from every side of the logo and route outward to glowing nodes, with
 * pulses flowing continuously from the logo. "POWERLINE" sits under the mark.
 */

// The brand P mark (same vector as the favicon).
const P_PATH =
...
// P centred at (500, 480) on a 1000×1000 stage.
const P_TF = "translate(232 215) scale(2.75)";

// Conductors emerge from every side of the P and route out to nodes.
const EDGES = [
  "M 388 358 C 330 305 245 255 150 150", // NW
  "M 500 332 C 514 250 486 180 500 92", // N
  "M 616 360 C 688 308 778 255 852 150", // NE
  "M 357 470 C 282 460 190 486 100 470", // W
  "M 646 468 C 730 468 814 456 900 466", // E
  "M 372 548 C 300 600 200 650 120 692", // WSW
  "M 632 548 C 704 600 802 650 882 692", // ESE
  "M 404 600 C 350 668 252 762 168 852", // SW
  "M 600 600 C 654 668 752 762 834 852", // SE
];

// Glowing nodes — path ends plus a few junctions along the way.
const NODES = [
  { x: 150, y: 150 }, { x: 500, y: 92 }, { x: 852, y: 150 },
  { x: 100, y: 470 }, { x: 900, y: 466 },
  { x: 120, y: 692 }, { x: 882, y: 692 },
  { x: 168, y: 852 }, { x: 834, y: 852 },
  { x: 255, y: 250 }, { x: 700, y: 250 },
  { x: 200, y: 474 }, { x: 272, y: 728 }, { x: 730, y: 728 },
];

export default function EnergyNetwork() {
  return (
    <div className="en" aria-hidden="true">
      <svg viewBox="0 0 1000 1000" preserveAspectRatio="xMidYMid meet" className="en-svg">
        {/* conductors */}
        {EDGES.map((d, i) => (
          <g key={`e${i}`} style={{ "--d": `${1.0 + i * 0.08}s` }}>
        ...
        {/* nodes */}
        {NODES.map((n, i) => (
          <g key={`n${i}`} style={{ "--d": `${1.5 + i * 0.04}s` }}>
            <circle className="en-ring" cx={n.x} cy={n.y} r="12" pathLength="1" />
            <circle className="en-node" cx={n.x} cy={n.y} r="6" />
          </g>
        ))}

        {/* the logo = the power source (grows in, charges, then powers out) */}
        <g className="en-p-grow">
          <g transform={P_TF}>
        ...
        <text className="en-word" x="500" y="688" textAnchor="middle">
      ...
      <style jsx>{`
        ...
        /* ── the logo grows in, then glows ── */
        .en-p-grow {
          transform-box: fill-box;
          transform-origin: center;
          animation: enGrow 0.9s cubic-bezier(0.34, 1.4, 0.5, 1) 0.1s both;
        }
        @keyframes enGrow {
        ...
        @media (prefers-reduced-motion: reduce) {
          .en-path, .en-flow, .en-node, .en-ring, .en-p-grow,
