...
function buildAcquisition(rows) {
  ...
  for (const row of all) {
    const b = bucketSource(row.source, row.medium);
    buckets[b] = (buckets[b] || 0) + row.sessions;
  }
  const bySource = Object.entries(buckets)
    .map(([source, sessions]) => ({ source, sessions, pct: total ? (sessions / total) * 100 : 0 }))
    .sort((a, b) => b.sessions - a.sessions);
  const drill = all
    .sort((a, b) => b.sessions - a.sessions)
    .slice(0, 15)
    .map((d) => ({ ...d, pct: total ? (d.sessions / total) * 100 : 0 }));
  return { total, bySource, drill };
}
