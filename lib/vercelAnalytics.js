...
export async function getVercelAnalytics({ days = 30, debug = false } = {}) {
  ...
  // ── TEMPORARY DIAGNOSTIC (?debug=1, admin-only) ───────────────────────────
  // Mirrors the NORMAL visits/count request — same env-based buildParams the
  // dashboard uses, NO override — to expose exactly which projectId/team scope
  // the live env resolves to and what Vercel returns. Safe fields only: no
  // token, headers, cookies, or raw secrets. REMOVE once the 0 is resolved.
  let probe;
  if (debug) {
    const team = process.env.VERCEL_TEAM_ID;
    probe = {
      ...
      teamScope: team ? (team.startsWith("team_") ? `teamId=${team}` : `slug=${team}`) : "none",
    };
    try {
      ...
      probe.visitors = j?.data?.visitors ?? null;
      probe.pageviews = j?.data?.pageviews ?? null;
    } catch (e) {
  ...
  return {
    ...
    // TEMPORARY: present only when ?debug=1 — normal-path scope + parsed result.
    ...(probe ? { probe } : {}),
  };
}
