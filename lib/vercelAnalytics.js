...
// never placed in a URL, never logged). Optional VERCEL_TEAM_ID for team
// accounts (omitted on personal accounts). ANALYTICS_TARGET_PROJECT_ID names
// the website project to query — a CUSTOM var, NOT Vercel's system
// VERCEL_PROJECT_ID (which is auto-set to this CRM deployment's own id). Any
// metric the API doesn't return degrades to null → the UI shows "Not available
// from Vercel API" rather than inventing data.
// ─────────────────────────────────────────────────────────────────────────

const API = "https://api.vercel.com/v1/query/web-analytics";

export function isVercelConfigured() {
  return Boolean(process.env.VERCEL_API_TOKEN && process.env.ANALYTICS_TARGET_PROJECT_ID);
}
...
function buildParams(extra) {
  ...
  // ANALYTICS_TARGET_PROJECT_ID (custom) — deliberately NOT VERCEL_PROJECT_ID,
  // which is a Vercel system var auto-set to THIS deployment's own project id
  // and would make us query the CRM app instead of the target website.
  p.set("projectId", process.env.ANALYTICS_TARGET_PROJECT_ID);
 ...
 */
export async function getVercelAnalytics({ days = 30, start, end } = {}) {
  if (!isVercelConfigured()) return { configured: false };

  // Explicit YYYY-MM-DD start/end (from the dashboard date filter) win; otherwise
  // fall back to a trailing `days` window ending today.
  const untilD = new Date();
  const sinceD = new Date(untilD.getTime() - days * 86400000);
  const since = /^\d{4}-\d{2}-\d{2}$/.test(start || "") ? start : sinceD.toISOString().slice(0, 10);
  const until = /^\d{4}-\d{2}-\d{2}$/.test(end || "") ? end : untilD.toISOString().slice(0, 10);
  const diag = [];
