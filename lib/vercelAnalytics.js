...
function buildParams(extra, scope) {
  ...
  //
  // `scope` is an optional override (used only by the admin debug path): when
  // present, its projectId/team fully replace the env values for this one call.
  const team = scope ? scope.team : process.env.VERCEL_TEAM_ID;
  if (team) {
    if (team.startsWith("team_")) p.set("teamId", team);
    else p.set("slug", team);
  }
  p.set("projectId", scope ? scope.projectId : process.env.VERCEL_PROJECT_ID);
 ...
 */
export async function getVercelAnalytics({ days = 30, debug = false, debugScope } = {}) {
  ...
  // When the route passes debugScope (from ?projectId=/?teamId=/?slug=), the
  // debug fetch below queries THAT scope instead of the env vars — so an admin
  // can probe which project actually holds the data without a redeploy. This
  // affects ONLY this debug fetch; the real dashboard queries (totals/breakdown
  // below) always use the env config. REMOVE this override (and the route's
  // param parsing) once the correct VERCEL_PROJECT_ID is confirmed.
  // ──────────────────────────────────────────────────────────────────────────
  let dbg;
  if (debug) {
    const scope = debugScope
      ? { projectId: debugScope.projectId, team: debugScope.team || "" }
      : { projectId: process.env.VERCEL_PROJECT_ID, team: process.env.VERCEL_TEAM_ID || "" };
    const teamLabel = scope.team
      ? scope.team.startsWith("team_") ? `teamId=${scope.team}` : `slug=${scope.team}`
      : "none";
    const path = `visits/count?${buildParams({ since, until }, scope).toString()}`;
    dbg = {
      override: Boolean(debugScope),
      scope: { projectId: scope.projectId, team: teamLabel, since, until },
      requestPath: path,
    };
    try {
