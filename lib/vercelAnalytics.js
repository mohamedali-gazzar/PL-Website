...
function buildParams(extra) {
  ...
  // Team projects require teamId (or slug); personal projects omit it.
...
async function apiGet(path, extra) {
  ...
  // (which include only the path + status) can't leak it.
...
export async function getVercelAnalytics({ days = 30 } = {}) {
  ...
  const [tot, topPages, referrers, countries, devices, browsers, os] = await Promise.all([
  ...
  return {
    ...
    browsers,
    os,
    // Only surfaced (admin-only) when the whole source failed. Contains only
    // "path → HTTP status" strings — never the token.
