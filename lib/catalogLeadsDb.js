...
// Fixed, Excel-friendly column order for the CSV export.
const CSV_COLUMNS = [
...
// Format rows to an ordered matrix of plain strings (header row first) — shared
// by BOTH exports so the CSV and the XLSX always have identical columns/values.
// The source column always shows the human label ("Product Catalog"); booleans
// render Yes/No and dates as ISO. Pure + testable.
export function toRowsMatrix(rows = []) {
  ...
  return [header, ...rows.map((r) => CSV_COLUMNS.map(([key]) => cell(r, key)))];
}

/** Serialize catalogue-lead rows to CSV (header + rows). Pure + testable. */
export function toCsv(rows = []) {
  // Excel-safe: quote a cell if it contains a comma / quote / newline.
  const esc = (s) => (/[",\n\r]/.test(s) ? `"${String(s).replace(/"/g, '""')}"` : s);
  return toRowsMatrix(rows).map((row) => row.map(esc).join(",")).join("\r\n");
}
