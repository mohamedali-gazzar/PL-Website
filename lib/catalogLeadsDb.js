...
function csvCell(v) {
  ...
  // Excel-safe: quote if the cell contains a comma / quote / newline.
...
export function toCsv(rows = []) {
  ...
  const lines = rows.map((r) =>
    CSV_COLUMNS.map(([key]) =>
      // The source column always shows the human label ("Product Catalog").
      key === "source" ? csvCell(CATALOG_LEAD_SOURCE_LABEL) : csvCell(r[key])
    ).join(",")
  );
