...
import { isCatalogDbConfigured, listCatalogLeads } from "@/lib/catalogLeadsDb";
import CatalogLeadsBoard from "./CatalogLeadsBoard";
...
export const metadata = {
  title: "Catalogue Leads · Powerline",
  ...
  return (
    <div className="plshell">
      ...
      <div className="shell-card">
        ...
        <p className="muted">Enter the admin password to view catalogue leads.</p>
        ...
        <form method="POST" action="/api/admin/login">
          ...
          <button type="submit" disabled={!configured}>Unlock catalogue leads</button>
        </form>
      </div>
    </div>
  );
...
export default async function CatalogLeadsAdminPage({ searchParams }) {
  if (!isAuthed(cookies().get(ADMIN_COOKIE)?.value)) {
    return <LoginScreen error={searchParams?.error} configured={isConfigured()} />;
  }

  const configured = isCatalogDbConfigured();
  let leads = [];
  let dbError = null;
  if (configured) {
    try {
      leads = await listCatalogLeads();
    } catch (e) {
      dbError = e?.message || "Could not reach the catalogue leads database.";
    }
  }

  return <CatalogLeadsBoard initialLeads={leads} configured={configured} dbError={dbError} />;
}
